module jet_zonal_flow_test_mod
  ...
  real(r8), parameter :: alpha = 1.0 / 3.0_r8
  real(r8), parameter :: beta  = 1.0 / 15.0_r8
...
contains
  ...
  do j = mesh%full_lat_start_idx, mesh%full_lat_end_idx
    i = mesh%half_lon_start_idx
    if (j == mesh%full_lat_start_idx) then
      state%gd(i,j) = gh0
    else
      call qags(gh_integrand, -0.5*pi, mesh%full_lat(j), 1.0e-12, 1.0e-3, state%gd(i,j), abserr, neval, ierr)
      if (ierr /= 0) then
        call log_error('Failed to calculate integration at (' // to_string(i) // ',' // to_string(j) // ')!')
      end if
      state%gd(i,j) = gh0 - state%gd(i,j)
    end if
    do i = mesh%half_lon_start_idx, mesh%half_lon_end_idx
      state%gd(i,j) = state%gd(mesh%half_lon_start_idx,j)
      ! Add perturbation.
