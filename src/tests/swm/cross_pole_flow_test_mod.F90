module cross_pole_flow_test_mod
  ...
  real(r8), parameter :: v0 = 20 ! m s-1
  real(r8), parameter :: gd0 = 5.7684e4 ! m2 s-2
  ...
  subroutine cross_pole_flow_test_set_initial_condition(static, state)
    ...
    do j = mesh%full_lat_start_idx_no_pole, mesh%full_lat_end_idx_no_pole
      ...
      do i = mesh%half_lon_start_idx, mesh%half_lon_end_idx
        ...
        state%u(i,j) = -v0 * sin_lon * sin_lat * (4.0 * cos_lat**2 - 1.0) * mesh%full_cos_lat(j)
      end do
    end do
    ...
    do j = mesh%half_lat_start_idx_no_pole, mesh%half_lat_end_idx_no_pole
      ...
      do i = mesh%full_lon_start_idx, mesh%full_lon_end_idx
        ...
        state%v(i,j) = v0 * sin_lat**2 * cos_lon * mesh%half_cos_lat(j) 
      end do 
    end do 
